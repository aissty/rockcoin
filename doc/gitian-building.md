...
*Setup instructions for a Gitian build of RockCoin Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the RockCoin
...
- [Building RockCoin Core](#building-rockcoin-core)
...
Clone the git repositories for rockcoin and Gitian.
...
in the rockcoin repository under 'Fetch and create inputs' to install sources which require
manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache
and offline git repositories' which will fetch the remaining files required for building
offline.

Building RockCoin Core
----------------

To build RockCoin Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the rockcoin repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/rockcoin/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/rockcoin-project/rockcoin
...
For example:
```bash
URL=https://github.com/thrasher-/rockcoin.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit rockcoin=${COMMIT} --url rockcoin=${URL} ../rockcoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit rockcoin=${COMMIT} --url rockcoin=${URL} ../rockcoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit rockcoin=${COMMIT} --url rockcoin=${URL} ../rockcoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the rockcoin git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../rockcoin/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/rockcoin
SIGPATH=/some/root/path/rockcoin-detached-sigs

./bin/gbuild --url rockcoin=${BTCPATH},signature=${SIGPATH} ../rockcoin/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/rockcoin-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/rockcoin-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/rockcoin-osx-build.assert
```
...
[rockcoin-project/gitian.sigs.ltc](https://github.com/rockcoin-project/gitian.sigs.ltc/) repository, or if that's not possible create a pull
